...
function NewThreadModal({
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!form.raw_input.trim()) {
      toast.error("Cole o material que a IA deve tratar.");
      return;
    }
    onCreate({
      ...
      >
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          ...
          <Field
            ...
            hint="Cole o texto da peça, ata, e-mail, descrição livre — o que a IA vai organizar"
            full
          >
            <textarea
              required
          ...
          <div className="sm:col-span-2">
            <div className="flex items-center justify-between rounded-sm border border-dashed border-border bg-secondary/40 px-3 py-2 text-xs text-muted-foreground">
              <span className="flex items-center gap-2">
                <PaperclipIcon className="h-3.5 w-3.5" /> Upload de PDFs e documentos
              </span>
              <span className="text-[10px] uppercase tracking-wide opacity-60">em breve</span>
            </div>
          </div>
        </div>
